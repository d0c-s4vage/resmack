#ifndef RESMACK_ITEM_STR
#define RESMACK_ITEM_STR

#define STR(...) OVERLOADED_MACRO(STR, __VA_ARGS__)

#define STR_ALL(MIN, MAX) (new resmack::items::Str(MIN, MAX, resmack::items::CHARSET_ALL))

#define STR1(SIZE) STR2(SIZE, (SIZE)+1)
#define STR2(MIN, MAX) (new resmack::items::Str(MIN, MAX))
#define STR3(MIN, MAX, CHARSET) (new resmack::items::Str(MIN, MAX, CHARSET))

#include <cstdlib>
#include <iostream>

#include "resmack/item.hpp"
#include "resmack/defs.hpp"

namespace resmack {
namespace items {

UNUSED_DECL
const char CHARSET_ALL[] = "";
UNUSED_DECL
const char CHARSET_ALPHA[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
UNUSED_DECL
const char CHARSET_NUM[] = "0123456789";
UNUSED_DECL
const char CHARSET_PUNCTUATION[] = " !\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~";

class Str: public resmack::Item {
 private:
  uint32_t min_;
  uint32_t max_;
  uint32_t range_;
  std::string_view charset_;

 public:
  Str();
  Str(uint32_t min, uint32_t max);
  Str(uint32_t min, uint32_t max, std::string_view charset);

  ~Str();

  ItemType Type();
  void Build(BuildContext* ctx);
};

}
}

#endif
